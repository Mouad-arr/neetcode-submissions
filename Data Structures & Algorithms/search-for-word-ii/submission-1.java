...
class Solution {
    ...
    public void dfs(int rows,int cols ,int r , int c , TrieNode nood , String word,char[][] board, String[] words){
        ...
        dfs(rows,cols,r-1,c,node,newWord,board,words);
        dfs(rows,cols,r+1,c,node,newWord,board,words);
        dfs(rows,cols,r,c+1,node,newWord,board,words);
        dfs(rows,cols,r,c-1,node,newWord,board,words);
    ...
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for(int i=0;i<rows ;i++){
            for(int j=0;j<cols;j++){
                dfs(rows,cols,i,j,this.root,new String(),board,words);
            }}
